...
#include "mini_max_player_1.hpp"

namespace reversi
{

MiniMaxPlayer1::MiniMaxPlayer1(Side side) : Player(side){
    initializeWeightedSquare();
    initializeGamePhaseBoarder();
    initializeWeights();
}

CellPosition MiniMaxPlayer1::thinkNextMove(const Board &board)
{
    setStartTime();
    setNodeCount(0);
...
/*
//ディスク数で評価する評価関数
int MiniMaxPlayer1::evaluateFunction(const Board board)
{
...
float MiniMaxPlayer1::evaluateFunction(const Board board)
{
    float score;
...
//枝刈りありのMax
float MiniMaxPlayer1::maxNode(Board board, int depth, int max_depth, CellPosition &best_move,float alpha, float beta)
{
    addNodeCount();
...
float MiniMaxPlayer1::shallowMaxNode(Board board, int depth, int max_depth, CellPosition &best_move,float alpha, float beta)
{
    std::vector<CellPosition> legal_moves = board.getAllLegalMoves(getSide());
    std::vector<CellPosition> opponent_legal_moves=board.getAllLegalMoves(getOpponentSide(getSide()));
...
//枝刈りありのMini
float MiniMaxPlayer1::minNode(Board board, int depth, int max_depth, CellPosition &best_move,float alpha, float beta)
{
    //if(getSearchTimeMiniSec() > max_time){
    //    return beta;
    //}
...
float MiniMaxPlayer1::shallowMinNode(Board board, int depth, int max_depth, CellPosition &best_move,float alpha, float beta)
{
    if(getSearchTimeMiniSec() > max_time){
        return beta;
    }
...
void MiniMaxPlayer1::setNodeCount(int count){
    node_count = count;
}

int MiniMaxPlayer1::getNodeCount(){
    return node_count;
}

void MiniMaxPlayer1::addNodeCount(){
    node_count ++;
}

void MiniMaxPlayer1::initializeWeightedSquare(){
...
float MiniMaxPlayer1::calculateWeightSquare(const Board board)
{
    float score = 0;
...
float MiniMaxPlayer1::calculateSimplecount(const Board board)
{
     return (float)board.count(getOwnState(getSide()));
}
/*
float MiniMaxPlayer1::calculateMobility(std::vector<CellPosition>& opponent_legal_moves, std::vector<CellPosition>& legal_moves)
{
    return ((float)legal_moves.size() - (float)opponent_legal_moves.size()) * weights[getGamePhase(getTurnNumber())][1];
}
*/

float MiniMaxPlayer1::calculateMobility(const Board board){
    return (float)board.getAllLegalMoves(getSide()).size() - (float)board.getAllLegalMoves(getOpponentSide(getSide())).size();
}

void MiniMaxPlayer1::initializeGamePhaseBoarder(){
    game_phase_boarder = {0, 12, 36, 50, 58, 64};
}

void MiniMaxPlayer1::initializeWeights(){
    //{weighted squares, mobility, simple count}
    weights[0] = {(float)1.0, (float)0.0, (float)0.0};
    weights[1] = {(float)1.0, (float)0.0, (float)0.0};
    weights[2] = {(float)1.0, (float)0.0, (float)0.0};
    weights[3] = {(float)1.0, (float)0.0, (float)0.0};
    weights[4] = {(float)1.0, (float)0.0, (float)0.0};
}

void MiniMaxPlayer1::setTurnNumber(int turn){
    turn_number = turn;
}

int MiniMaxPlayer1::getTurnNumber(){
    return turn_number;
}

int MiniMaxPlayer1::getGamePhase(int turn){
    for(int i = 0 ; i < MiniMaxPlayer1::game_phase; i++){
...
bool MiniMaxPlayer1::endGameSearchNode(const Board board,Side side){
...
bool MiniMaxPlayer1::endGameSearch(const Board board, CellPosition& best_move)
{
...
int MiniMaxPlayer1::getSearchTimeMiniSec()
{
    std::chrono::system_clock::time_point end_time = std::chrono::system_clock::now();
    return (int)std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time).count();
}

void MiniMaxPlayer1::setStartTime()
{
    start_time = std::chrono::system_clock::now();
}

void MiniMaxPlayer1::setEndSearchNodeCount(int count){
    end_search_node_count = count;
}

int MiniMaxPlayer1::getEndSearchNodeCount(){
    return end_search_node_count;
}

void MiniMaxPlayer1::addEndSearchNodeCount(){
    end_search_node_count++;
}

void MiniMaxPlayer1::setMaxDepth(){
