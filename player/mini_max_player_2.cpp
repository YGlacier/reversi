...
#include "mini_max_player_2.hpp"

namespace reversi
{

MiniMaxPlayer2::MiniMaxPlayer2(Side side) : Player(side){
    initializeWeightedSquare();
    initializeGamePhaseBoarder();
    initializeWeights();
}

CellPosition MiniMaxPlayer2::thinkNextMove(const Board &board)
{
    setStartTime();
    setNodeCount(0);
...
/*
//ディスク数で評価する評価関数
int MiniMaxPlayer2::evaluateFunction(const Board board)
{
...
float MiniMaxPlayer2::evaluateFunction(const Board board)
{
    float score;
...
//枝刈りありのMax
float MiniMaxPlayer2::maxNode(Board board, int depth, int max_depth, CellPosition &best_move,float alpha, float beta)
{
    addNodeCount();
...
float MiniMaxPlayer2::shallowMaxNode(Board board, int depth, int max_depth, CellPosition &best_move,float alpha, float beta)
{
    std::vector<CellPosition> legal_moves = board.getAllLegalMoves(getSide());
    std::vector<CellPosition> opponent_legal_moves=board.getAllLegalMoves(getOpponentSide(getSide()));
...
//枝刈りありのMini
float MiniMaxPlayer2::minNode(Board board, int depth, int max_depth, CellPosition &best_move,float alpha, float beta)
{
    if(getSearchTimeMiniSec() > max_time){
        return beta;
    }
...
float MiniMaxPlayer2::shallowMinNode(Board board, int depth, int max_depth, CellPosition &best_move,float alpha, float beta)
{
    if(getSearchTimeMiniSec() > max_time){
        return beta;
    }
...
void MiniMaxPlayer2::setNodeCount(int count){
    node_count = count;
}

int MiniMaxPlayer2::getNodeCount(){
    return node_count;
}

void MiniMaxPlayer2::addNodeCount(){
    node_count ++;
}

void MiniMaxPlayer2::initializeWeightedSquare(){
...
float MiniMaxPlayer2::calculateWeightSquare(const Board board)
{
    float score = 0;
...
float MiniMaxPlayer2::calculateSimplecount(const Board board)
{
     return (float)board.count(getOwnState(getSide()));
}
/*
float MiniMaxPlayer2::calculateMobility(std::vector<CellPosition>& opponent_legal_moves, std::vector<CellPosition>& legal_moves)
{
    return ((float)legal_moves.size() - (float)opponent_legal_moves.size()) * weights[getGamePhase(getTurnNumber())][1];
}
*/

float MiniMaxPlayer2::calculateMobility(const Board board){
    return (float)board.getAllLegalMoves(getSide()).size()/* - (float)board.getAllLegalMoves(getOpponentSide(getSide())).size()*/;
}

void MiniMaxPlayer2::initializeGamePhaseBoarder(){
    game_phase_boarder = {0, 12, 36, 50, 58, 64};
}

void MiniMaxPlayer2::initializeWeights(){
    //{weighted squares, mobility, simple count}
    weights[0] = {(float)1.0, (float)2.5, (float)0.0};
    weights[1] = {(float)1.0, (float)2.0, (float)0.0};
    weights[2] = {(float)1.0, (float)1.5, (float)0.0};
    weights[3] = {(float)1.0, (float)1.5, (float)0.0};
    weights[4] = {(float)1.0, (float)1.5, (float)5.0};
}

void MiniMaxPlayer2::setTurnNumber(int turn){
    turn_number = turn;
}

int MiniMaxPlayer2::getTurnNumber(){
    return turn_number;
}

int MiniMaxPlayer2::getGamePhase(int turn){
    for(int i = 0 ; i < MiniMaxPlayer2::game_phase; i++){
...
bool MiniMaxPlayer2::endGameSearchNode(const Board board,Side side){
...
bool MiniMaxPlayer2::endGameSearch(const Board board, CellPosition& best_move)
{
...
int MiniMaxPlayer2::getSearchTimeMiniSec()
{
    std::chrono::system_clock::time_point end_time = std::chrono::system_clock::now();
    return (int)std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time).count();
}

void MiniMaxPlayer2::setStartTime()
{
    start_time = std::chrono::system_clock::now();
}

void MiniMaxPlayer2::setEndSearchNodeCount(int count){
    end_search_node_count = count;
}

int MiniMaxPlayer2::getEndSearchNodeCount(){
    return end_search_node_count;
}

void MiniMaxPlayer2::addEndSearchNodeCount(){
    end_search_node_count++;
}

void MiniMaxPlayer2::setMaxDepth(){
    if(getTurnNumber() > 0 && getTurnNumber() <= 16){
        max_depth = 7;
    }else if(getTurnNumber() > 16 && getTurnNumber() <=36){
        max_depth = 7;
